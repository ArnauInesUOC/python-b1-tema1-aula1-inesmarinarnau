...
def mult_recursive(value, times):
    result = 0
    if type(value) != int:
        raise ValueError("El parámetro 'value' debe ser un número entero")
    elif type(times) != int:
        raise ValueError("El parámetro 'times' debe ser un número entero")
    elif times == 1:
        return value
    else:
        result = value + mult_recursive(value, (times - 1)) 
