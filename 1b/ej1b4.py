...
def results(list_numbers):
    if list_numbers == []:
        raise ValueError("El parámetro de entrada no puede estar vacío")
    for numbers in list_numbers:
        if not isinstance(numbers, (int, float)):
            raise ValueError("El parámetro de entrada debe ser una lista de números enteros y/o decimales")
    mean = round(np.mean(list_numbers),2)
    sd = round(np.std(list_numbers),2)
